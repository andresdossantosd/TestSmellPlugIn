package upm.es.testsmell.smell;
...
import upm.es.testsmell.Util;
import upm.es.thresholds.Thresholds;
    ...
    public void runAnalysis(CompilationUnit testFileCompilationUnit, CompilationUnit productionFileCompilationUnit, String testFileName, String productionFileName) throws FileNotFoundException {
        ...
        for (TestMethod testMethod : testMethods) {
            ...
            if (testMethod.getCalledMethods().stream().anyMatch(x -> x.getName().equals(testMethods.stream().map(z -> z.getMethodDeclaration().getNameAsString())))) {
                smellyElementsSet.add(new upm.es.testsmell.TestMethod(testMethod.getMethodDeclaration().getNameAsString()));
            }
        }
